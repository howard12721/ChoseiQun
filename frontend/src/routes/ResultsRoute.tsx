...
import {
  ...
  deleteComment,
  updateComment,
} from "../features/poll-results/comments/api";
import {
  EMPTY_COMMENT_DRAFT,
  type CommentDraft,
} from "../features/poll-results/comments/model";
...
export function ResultsRoute({ pollId }: { pollId: string }) {
  ...
  const [pendingAction, setPendingAction] =
    useState<PendingCommentAction>(null);
    ...
    <Shell flash={flash} onDismissFlash={dismissFlash}>
      ...
      {!resource.loading && !resource.error && resource.data ? (
        <ResultsPage
          ...
          onEditComment={(editingCreatedAt, body) =>
            setDraft({ body, editingCreatedAt })
          }
          onCancelCommentEdit={() => setDraft(EMPTY_COMMENT_DRAFT)}
          onDeleteComment={(createdAt) => {
            void removeComment(createdAt).catch((caught) =>
